    ...
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto min-h-[70vh]">
      ...
      <div className="w-full flex flex-col gap-8 pb-20">
        {narutoCharacters.map((char, index) => (
          <motion.div 
            key={index} 
            className="bg-white rounded-[2rem] p-8 md:p-10 shadow-[0_8px_30px_rgb(225,29,72,0.06)] border border-rose-100 hover:border-rose-200 transition-colors"
          ...
          >
            ...
            <div className="flex flex-col border-b border-slate-100 pb-6 mb-8 text-center md:text-left">
              ...
              <h3 className="text-xl text-rose-600 font-bold tracking-wide">{char.name_romaji}</h3>
            </div>
              ...
              <div className="flex flex-col gap-4">
                {char.explanation.map((item, i) => (
                  <div key={i} className="flex flex-col md:flex-row gap-2 md:gap-6 p-5 rounded-2xl bg-slate-50 border border-slate-100 items-start">
                    <div className="text-2xl font-bold text-rose-600 min-w-[140px]">
                      {item.kanji}
                    </div>
            ...
            <div className="bg-rose-50 border-l-4 border-rose-500 p-6 rounded-r-2xl">
              <p className="text-base text-rose-900 italic font-medium leading-relaxed">"{char.summary}"</p>
            </div>
